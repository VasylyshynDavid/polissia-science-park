...
<head>
    ...
    <title>@yield('title', 'Адмін-панель') — Science Park CMS</title>
    <style>
...
<body>
    <div class="admin-shell">
        <aside class="sidebar">
            <div class="brand">
                <img src="{{ asset('images/logo-science-park-monogram.png') }}" alt="Science Park">
                <div>Science Park<br>CMS</div>
            </div>
            <nav class="nav">
                <a href="{{ route('home', ['locale' => 'uk']) }}" target="_blank">Публічний сайт</a>
                <a href="{{ route('admin.dashboard') }}">Панель</a>
                <a href="{{ route('admin.sliders.index') }}">Слайдер</a>
                <a href="{{ route('admin.activities.index') }}">Напрями діяльності</a>
                <a href="{{ route('admin.opportunities.index') }}">Наші можливості</a>
                <a href="{{ route('admin.news.index') }}">Новини</a>
                <a href="{{ route('admin.categories.index') }}">Категорії новин</a>
                <form method="post" action="{{ route('admin.logout') }}">@csrf<button type="submit">Вийти</button></form>
            </nav>
        </aside>
        <main class="content">
            @if(session('success'))<div class="success">{{ session('success') }}</div>@endif
            @if($errors->any())
                <div class="errors"><b>Перевірте поля:</b><ul>@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>
            @endif
            @yield('content')
        </main>
    </div>
